...
int main()  
{
	 my_uuid = (char*) malloc(256);  
	
    pthread_t thread_id;  
	 THREAD st;					// 2가지 변수를 넘겨줄 구조체 선언
	 st.mclient = 0;			// 구조체 멤버 초기화
	 st.muuid = (char*) malloc(256);	// 구조체 멤버 메모리 할당.
    ...
    strcpy(st.muuid, my_uuid); 		// 구조체 멤버에 uuid 넣기. 
    // allocate socket
    sock = socket(AF_BLUETOOTH, SOCK_STREAM, BTPROTO_RFCOMM);
    printf("Bluetooth server start.\n");
    ...
    while(1)
    {
        ...
        st.mclient = accept(sock, (struct sockaddr *)&rem_addr, &opt);
    
        ba2str(&rem_addr.rc_bdaddr, buffer);
        fprintf(stderr, "accepted connection from %s\n", buffer);
        memset(buffer, 0, sizeof(buffer));
        
        pthread_create( &thread_id, NULL, ThreadMain, (void*)&st);   
    }
...
{  
    char buf[1024];  
 	 THREAD* pp = (THREAD*)argument;		// 구조체 변수 선언
	 int client = pp->mclient;				// main에서 받아온 client 멤버
	 char* uuid = pp->muuid;				// main에서 받아온 uuid멤버
    ...
    {  
        ...
        printf("%s\n", uuid);				// uuid 서버에 출력
        write_server(client, uuid);		// uuid client에 전송
    }  
